...
use crate::utils;
use crate::utils::{LOGIC_SIZE, LOGIC_SIZE_I64, LOGIC_SIZE_U32};
...
impl DirectoryEntry {
        ...
        T: Write,
    {
        for entry in &mut self.dir_childs {
            DirectoryEntry::write_path_table_entry::<T, Order>(
                entry,
                output_writter,
                0,
            )?;
        ...
        T: Write + Seek,
    {
        ...
        output_writter.seek(SeekFrom::Start(u64::from(
            path_table_pos * LOGIC_SIZE_U32,
        )))?;

        let old_pos_current_context = output_writter.stream_position()?;
        ...
        T: Write + Seek,
    {
        ...
        // Seek to the correct LBA
        output_writter
            .seek(SeekFrom::Start(u64::from(self.lba * LOGIC_SIZE_U32)))?;

        let directory_type_current =
            if parent_option.is_none() { 3 } else { 1 };
    ...
    pub fn get_file(&mut self, path: &str) -> Option<&mut FileEntry> {
        ...
        if cut_path.len() != 1 {
            directory_entry =
                directory_entry.unwrap().get_directory(cut_path[0]);
    ...
    fn add_and_merge_childs_directories(
        ...
        let optinal_present_entry: Option<&mut DirectoryEntry> = dir_childs
            .iter_mut()
            .filter(|in_entry| {
                in_entry.get_file_name() == new_entry.get_file_name()
            })
            .last();
    ...
    fn merge_child_directories(&mut self, other: DirectoryEntry) {
        for mut child in other.dir_childs {
            ...
            let optinal_present_entry: Option<&mut DirectoryEntry> = vec
                .iter_mut()
                .filter(|in_entry| {
                    in_entry.get_file_name() == child.get_file_name()
                })
                .last();
            if let Some(present_entry) = optinal_present_entry {
    ...
    pub fn set_path(&mut self, path: &[PathBuf]) -> std::io::Result<()> {
        ...
        for path in path.iter().filter(|path| path.is_file()) {
