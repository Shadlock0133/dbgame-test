...
impl WavHeaderFormat {
    pub fn read(fs: &mut FileStream) -> WavHeaderFormat {
        ...
        fs.read_exact(&mut fmt_chunk_marker)
            .expect("Failed reading header");
        let length_of_fmt = fs
            .read_u32::<LittleEndian>()
            .expect("Failed reading header");
        let format_type = fs
...
impl SoundDriver {
    ...
    pub fn new(max_voices: usize) -> SoundDriver {
        assert!(
            ...
            "Cannot init sound driver with more than {} voices",
            VOICE_COUNT
        );
        ...
        for i in 0..max_voice {
            ...
            if !voice._is_playing && !get_voice_state(voice.slot) {
                ...
                match ret {
                    Some(r) => {
                        let rv = &voices[r];
                        if voice.play_time < rv.play_time
    ...
    fn assign_hw_voice(
        ...
        let voice = SoundDriver::allocate_voice(
            voices,
            search_offset,
            max_voice,
            emitter.priority,
    ...
    fn calc_3d(
        ...
        let dist = Vector3::distance(position, listener_position)
            .clamp(atten_min_dist, atten_max_dist);
        let gain = match atten_type {
            AttenuationType::Linear => {
                1.0 - atten_rolloff * (dist - atten_min_dist)
                    / (atten_max_dist - atten_min_dist)
            }
            AttenuationType::InverseDistance => {
                atten_min_dist
                    / (atten_min_dist + atten_rolloff * (dist - atten_min_dist))
            }
            AttenuationType::ExponentialDistance => {
                (dist / atten_min_dist).powf(-atten_rolloff)
            }
            AttenuationType::None => 1.0,
        };
    ...
    fn update_voice(
        ...
        if emitter.id == voice.id {
            ...
            if emitter.is_3d {
                let (gain3d, pan3d) = SoundDriver::calc_3d(
                    listener_position,
                    listener_orientation,
                    &emitter.position,
                    emitter.atten_type,
                    emitter.atten_min_dist,
                    emitter.atten_max_dist,
                    emitter.atten_rolloff,
                );
            ...
            queue_set_voice_param_i(
                voice_slot,
                AudioVoiceParam::SampleData,
                emitter.sample.handle,
                t,
            );
            queue_set_voice_param_i(
                voice_slot,
                AudioVoiceParam::Samplerate,
                emitter.sample.samplerate,
                t,
            );
            queue_set_voice_param_i(
                voice_slot,
                AudioVoiceParam::LoopEnabled,
                if emitter.looping { 1 } else { 0 },
                t,
            );
            queue_set_voice_param_i(
                voice_slot,
                AudioVoiceParam::LoopStart,
                0,
                t,
            );
            ...
            queue_set_voice_param_i(
                voice_slot,
                AudioVoiceParam::Reverb,
                if emitter.reverb { 1 } else { 0 },
                t,
            );
            queue_set_voice_param_f(
                voice_slot,
                AudioVoiceParam::Volume,
                gain,
                t,
            );
            queue_set_voice_param_f(
                voice_slot,
                AudioVoiceParam::Detune,
                0.0,
                t,
            );
            queue_set_voice_param_f(
                voice_slot,
                AudioVoiceParam::Pitch,
                emitter.pitch,
                t,
            );
            ...
            queue_set_voice_param_f(
                voice_slot,
                AudioVoiceParam::FadeInDuration,
                0.0,
                t,
            );
            queue_set_voice_param_f(
                voice_slot,
                AudioVoiceParam::FadeOutDuration,
                0.0,
                t,
            );
        } else {
    ...
    pub fn update(&mut self) {
        ...
        for emitter_rc in emitters {
            let voice = { emitter_rc.read().unwrap().voice };
            {
                ...
                match voice {
                    ...
                    None => {
                        if emref.looping {
                            SoundDriver::assign_hw_voice(
                                &self.listener_position,
                                &self.listener_orientation,
                                &mut self.voices,
                                &mut self.search_offset,
                                self.max_voices,
                                &mut emref,
                            );
                        }
        ...
        self.emitters.retain(|x| x.read().unwrap().is_valid);
    }
